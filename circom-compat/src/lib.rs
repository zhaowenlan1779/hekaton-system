...
impl<F: PrimeField> R1CSFile<F> {
    ...
    // This requires that the constraint system has been finalized.
    pub fn from_cs_slow(cs: ConstraintSystemRef<F>) -> Result<Self, SynthesisError> {
        let matrices = cs.to_matrices().unwrap();
        let header = Header {
            field_size: 32,
            prime_size: hex::decode("010000f093f5e1439170b97948e833285d588181b64550b829a031e1724e6430")
                .unwrap(),
            n_wires: (cs.num_instance_variables() + cs.num_witness_variables()) as u32,
            n_pub_out: 0,
            n_pub_in: cs.num_instance_variables() as u32,
            n_prv_in: cs.num_witness_variables() as u32,
            n_labels: 0,
            n_constraints: cs.num_constraints() as u32,
        };
        ...
        Ok(Self {
            version: 1,
            header,
            constraints: matrices.a.iter().zip(matrices.b.iter()).zip(matrices.c.iter())
                .map(|((lc_a, lc_b), lc_c)| {
                    (lc_a.iter().cloned().map(|(x, y)| (y, x)).collect::<Vec<_>>(), lc_b.iter().cloned().map(|(x, y)| (y, x)).collect::<Vec<_>>(), lc_c.iter().cloned().map(|(x, y)| (y, x)).collect::<Vec<_>>())
                })
                .collect(),
            wire_mapping: vec![],
            witness: vec![&cs.instance_assignment[..], &cs.witness_assignment[..]].concat(),
        })
    }
}
